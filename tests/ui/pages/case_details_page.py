...
from tests.ui.factories.case_data_factory import SlaPlanData, SupportPlanData
...
class CaseDetailsPage(BasePage):
    ...
    def expect_sla_plan(self, data: SlaPlanData) -> None:
        expect(self.page.get_by_role("heading", name="SLA обработки")).to_be_visible()
        expect(self.page.get_by_text(data.urgency)).to_be_visible()
        expect(self.page.get_by_text(data.expected_status)).to_be_visible()
        expect(self.page.get_by_text(data.sla_comment)).to_be_visible()
